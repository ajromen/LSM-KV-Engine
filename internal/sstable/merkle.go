...
func (mt *MerkleTree) ValidateBlock(blockIndex uint32, blockData []byte) bool {
	hash := HashDataBlock(blockData)
	proof, err := mt.GetProof(blockIndex)
	if err != nil {
		return false
	}
...
func VerifyProof(proof *MerkleProof) bool {
	...
	if len(proof.Steps) == 0 {
		return proof.BlockHash == proof.RootHash
	}
	...
	for _, step := range proof.Steps {
		...
		if step.IsLeft {
			...
			copy(combined[0:32], step.Hash[:])
			copy(combined[32:64], currentHash[:])
		} else {
			// sibling is on the right
			copy(combined[0:32], currentHash[:])
			copy(combined[32:64], step.Hash[:])
		}
