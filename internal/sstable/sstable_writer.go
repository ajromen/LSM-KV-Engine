...
func (w *SSTableWriter) Finalize() error {
	...
	dictData := w.valueEncoder.SaveDict()
	dictOffset, dictSize, err := w.storage.WriteSegment(config.SegmentDictionary, dictData)
	if err != nil {
		return err
	}
	w.footer.DictionaryHandler = SegmentHandler{
		Offset: dictOffset,
		Size:   dictSize,
	}
